...
public class AssistantService {
    ...
    public ListQuestionResponse question(List<MultipartFile> files, AIQuestionRequest request) {
        ...
        files.forEach(file -> {
            ...
            String fileContentType = switch (Objects.requireNonNull(file.getContentType())) {
                case "image/jpeg", "image/png" -> "input_image";
                case "application/pdf" -> "input_file";
                default -> throw new IllegalStateException("Unexpected value: " + Objects.requireNonNull(file.getContentType()));
            };

            fileList.add(
                Map.of("s3FileUrl", s3FileUrl, "contentType", fileContentType)
            );
        });
...
//                    )
        ...
        try {
            ...
            System.out.println("AI: " + jsonNode.get("output").get(1).get("content").get(0).get("text").asText());
